...
export const CardContainer = (props: Props) => {
    ...
    useLayoutEffect(() => {
        setThemeSwitcher(themeConfig[theme].cardContainer);
    }, [theme]);
...
export const CardTitle = ({ children }: { children: ReactNode }) => {
    ...
    useLayoutEffect(() => {
        setThemeSwitcher(themeConfig[theme].cardTitle);
    }, [theme]);

    return (
        <h2 className={`transition-all duration-300 mb-6 text-xl ${themeSwitcher} font-semibold`}>
            {children}
        </h2>
    );
};
...
export const ProjectsContainer = ({ children }: { children: ReactNode }) => {
    ...
    useLayoutEffect(() => {
        setThemeSwitcher(themeConfig[theme].projectsContainer);
    }, [theme]);
...
export const TextSkeleton = ({ children }: { children: ReactNode }) => {
    const { theme } = useContext(ThemeContext);
    const [themeSwitcher, setThemeSwitcher] = useState({
        color: ""
    });
    ...
    useLayoutEffect(() => {
        setThemeSwitcher({
            color: themeConfig[theme].skeleton.color
        });
    }, [theme]);
    ...
    return (
        <div className="animate-pulse my-0 overflow-hidden">
            <div
                className={`transition-all duration-300 ${themeSwitcher.color} my-0 inline-block px-2 rounded text-gray-900/0 text-xs h-3 truncate`}
            >
                {children}
            </div>
        </div>
